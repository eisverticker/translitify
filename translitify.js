...
function translitifyBase(textarea, profile){
	...
	var locked = false;//no transliteration if locked
	var writeUpperCase = false;
	
	//handling "shift + .." characters in chrome/opera?
	// with keypress-methode
	var isCharMissed = false;
	
	var temporaryLetterExists = false;//for letter-replacement
	var currentNode = profile;
	...
	function insertAtCaret(letter){
		...
		//newer browsers	
			...
			textarea.value = val.substr(0,start)
							+letter
							+val.substr(textarea.selectionEnd);
